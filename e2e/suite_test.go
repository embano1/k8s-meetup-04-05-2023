...
func TestSimpleE2E(t *testing.T) {
	ctrl := features.New("e2e demo with nats").
		WithLabel("feature", "nats-e2e").
		Setup(setupNats()).
...
func publisherRunning() features.Func {
	return func(ctx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
		...
		ready := conditions.New(cfg.Client().Resources()).DeploymentConditionMatch(&publisher, appsv1.DeploymentAvailable, corev1.ConditionTrue)
